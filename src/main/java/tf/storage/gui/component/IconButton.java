...
public class IconButton extends GuiButton
{
    ...
    public IconButton(int id, int x, int y, int w, int h, int u, int v, ResourceLocation texture)
    {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float p_191745_4_)
    {
        if (this.visible)
        {
            ...
            this.drawTexturedModalRect(this.x, this.y,
                    this.getU() + (state - 1) * this.hoverOffsetU,
                    this.getV() + (state - 1) * this.hoverOffsetV, this.width, this.height);
