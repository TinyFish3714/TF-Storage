...
{
    public static void dropInventoryContentsInWorld(Level level, BlockPos pos, IItemHandler inv)
    {
        int slots = inv.getSlots();
        for (int i = 0; i < slots; i++) {
            ItemStack stack = inv.getStackInSlot(i);
            if (!stack.isEmpty()) {
                EntityHelper.dropItemStacksInWorld(level, pos, stack, true);
            }
        }
    }
    ...
    public static ItemStack tryInsertItemStackToInventoryWithinSlotRange(IItemHandler inv, @Nonnull ItemStack stackIn, SlotRange slotRange, boolean simulate)
    {
        ...
        // First pass: try to insert into existing stacks
    ...
    public static void leaveOneFullStackOfEveryItemWithinSlotRange(IItemHandler invTarget, IItemHandler invStorage, SlotRange slotRange, boolean reverse)
    {
        ...
        for (int slot = start; (reverse ? slot >= end : slot <= end); slot += inc)
        {
            ...
            int maxSize;
            if (invTarget instanceof IItemHandlerSize sizeHandler) {
                maxSize = sizeHandler.getItemStackLimit(slot, stack);
            } else {
                maxSize = invTarget.getSlotLimit(slot);
            }
    ...
    private static void sortInventoryInternal(IItemHandlerModifiable inv, SlotRange range)
    {
        ...
        try
        {
            ...
            {
                if (type.getStack().getItem() instanceof BlockItem) blocks.add(type);
                else items.add(type);
            }
            
            java.util.Comparator<ItemType> comparator = (t1, t2) -> {
                ResourceLocation r1 = ForgeRegistries.ITEMS.getKey(t1.getStack().getItem());
                ResourceLocation r2 = ForgeRegistries.ITEMS.getKey(t2.getStack().getItem());
        ...
        {
            TFStorageMod.LOGGER.error("库存排序时发生严重错误！正在恢复备份...", e);
    ...
    private static int fillSortedItems(IItemHandlerModifiable inv, List<ItemType> types, java.util.Map<ItemType, Integer> countMap, int startSlot, int endSlot)
    {
        ...
        for (ItemType type : types)
        {
            ...
            if (inv instanceof IItemHandlerSize sizeHandler) {
                effectiveStackLimit = sizeHandler.getItemStackLimit(currentSlot, template);
            } else {
            ...
            while (totalCount > 0)
            {
                ...
                {
                    TFStorageMod.LOGGER.warn("StackHelper: 排序溢出！丢失了 " + totalCount + " 个 " + template.getHoverName().getString());
                    break;
                }
